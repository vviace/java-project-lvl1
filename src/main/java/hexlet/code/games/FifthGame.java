...
public class FifthGame {
    public static void progression(int numberOfGames) {
        ...
        while (answerCount <= numberOfGames-1) {
            int firstElement = Engine.getRandomNumber(100);
            int operator = Engine.getRandomOperator();
            Random random = new Random();
            int progressionValue = random.nextInt(1,10);
            ...
            System.out.println("Question: " + progressionFirst.substring(1,progressionFirst.length()-1) + " .. " + progressionSecond.substring(1,progressionSecond.length()-1));
